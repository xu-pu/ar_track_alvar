...
void inline DrawLines(IplImage* image, const std::vector<PointType>& points, CvScalar color, bool loop = true)
{
  for (unsigned i = 1; i < points.size(); ++i)
    cvLine(image, cvPoint((int)points[i - 1].x, (int)points[i - 1].y), cvPoint((int)points[i].x, (int)points[i].y),
           color);
  if (loop)
  {
    cvLine(image, cvPoint((int)points[points.size() - 1].x, (int)points[points.size() - 1].y),
