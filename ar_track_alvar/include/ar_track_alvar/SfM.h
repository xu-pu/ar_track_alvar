...
namespace alvar
{
/** \brief Simple structure from motion implementation using \e CameraEC , \e
 * MarkerDetectorEC and \e TrackerFeaturesEC
 *
...
{
public:
  /** \brief Extended version of \e ExternalContainer structure used internally
   * in \e SimpleSfM */
...
public:
  ...
  /** \brief Reset the situation back to the point it was when \e SetResetPoint
   * was called */
  void Reset(bool reset_also_triangulated = true);
  /** \brief Remember the current state and return here when the \e Reset is
   * called */
  void SetResetPoint();
  /** \brief Clear all tracked features */
  void Clear();
  /** \brief Set the suitable scale to be used. This affects quite much how the
   * tracking behaves (when features are triangulated etc.) */
  void SetScale(double s)
  {
    scale = s;
  }
  /** \brief Get the camera used internally. You need to use this to set correct
   * camera calibration (see \e SamplePointcloud) */
  CameraEC* GetCamera();
  /** \brief Get the estimated pose */
  Pose* GetPose();
  /** \brief Add \e MultiMarker from file as a basis for tracking. It is good
   * idea to call \e SetResetPoint after these. */
  bool AddMultiMarker(const char* fname, FILE_FORMAT format = FILE_FORMAT_XML);
  /** \brief Add \e MultiMarker to be a basis for tracking. It is good idea to
   * call \e SetResetPoint after these. */
  bool AddMultiMarker(MultiMarkerEC* mm);
  /** \brief Add an marker to be a basis for tracking. It is good idea to call
   * \e SetResetPoint after these. */
  void AddMarker(int marker_id, double edge_length, Pose& pose);
  /** \brief Update position assuming that camera is moving with 6-DOF */
  bool Update(IplImage* image, bool assume_plane = true, bool triangulate = true, float reproj_err_limit = 5.f,
              float triangulate_angle = 15.f);
  /** \brief Update camera 6-DOF position using triangulated points only (This
   * is the old version of Update) */
  bool UpdateTriangulateOnly(IplImage* image);
  /** \brief Update position assuming that user is more standing still and
   * viewing the environment. */
  bool UpdateRotationsOnly(IplImage* image);
  /** \brief Draw debug information about the tracked features and detected
   * markers. */
