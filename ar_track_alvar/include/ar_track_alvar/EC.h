...
public:
  /** \brief Constructor */
  TrackerFeaturesEC(int _max_features = 100, int _min_features = 90, double _quality_level = 0.01,
                    double _min_distance = 10, int _pyr_levels = 4, int win_size = 6)
  ...
  template <typename T>
  bool Track(IplImage* img, IplImage* mask, std::map<int, T>& container, int type_id = -1, int first_id = -1,
             int last_id = -1)
  {
  ...
  template <typename T, typename F>
  bool Track(IplImage* img, IplImage* mask, std::map<int, T>& container, F do_handle_test, int type_id = 0,
             int first_id = -1, int last_id = -1)
  {
...
public:
  ...
  template <typename T>
  float Reproject(std::map<int, T>& container, Pose* pose, int type_id = -1, bool needs_has_p2d = false,
                  bool needs_has_p3d = false, float average_outlier_limit = 0.f)
  {
   ...
   */
  template <typename T>
  int EraseUsingReprojectionError(std::map<int, T>& container, float reprojection_error_limit, int type_id = -1,
                                  Pose* pose = 0)
  {
    if (pose)
  ...
  bool ReconstructFeature(const Pose* pose1, const Pose* pose2, const CvPoint2D32f* u1, const CvPoint2D32f* u2,
...
public:
  ...
  template <typename T>
  int Detect(IplImage* image, Camera* cam, std::map<int, T>& container, int type_id = 0, int first_id = 0,
             int last_id = 65535, bool track = false, bool visualize = false, double max_new_marker_error = 0.08,
             double max_track_error = 0.2, LabelingMethod labeling_method = CVSEQ)
  {
    int ret;
    ...
    ret = MarkerDetector<M>::Detect(image, cam, track, visualize, max_new_marker_error, max_track_error,
  ...
  template <typename T>
  void MarkerToEC(std::map<int, T>& container, int marker_id, double edge_length, Pose& pose, int type_id = 0,
...
public:
  ...
  template <typename T>
  bool Save(std::map<int, T>& container, const char* fname, FILE_FORMAT format = FILE_FORMAT_DEFAULT, int type_id = 0,
            int first_id = 0, int last_id = 65535)
  {
  ...
  template <typename T>
  bool Load(std::map<int, T>& container, const char* fname, FILE_FORMAT format = FILE_FORMAT_DEFAULT, int type_id = 0,
            int first_id = 0, int last_id = 65535)
  {
