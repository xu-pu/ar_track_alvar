...
void TrackerOrientation::Project(CvMat* state, CvMat* projection, void* param)
{
  ...
  cvProjectPoints2(tracker->_object_model, &rot_mat, &zero_tra, &(tracker->_camera->calib_K),
                   &(tracker->_camera->calib_D), projection);
...
bool TrackerOrientation::UpdateRotationOnly(IplImage* gray, IplImage* image)
{
  ...
  while (it != _F_v.end())
  {
    ...
    if (f->status3D == Feature::USE_FOR_POSE || f->status3D == Feature::IS_INITIAL)
    {
      ...
      if (image)
        cvLine(image, cvPoint(int(p2d[0]), int(p2d[1])), cvPoint(int(f->point.x), int(f->point.y)),
               CV_RGB(255, 0, 255));
