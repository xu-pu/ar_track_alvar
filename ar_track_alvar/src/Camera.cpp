...
bool ProjPoints::AddPointsUsingChessboard(IplImage* image, double etalon_square_size, int etalon_rows,
                                          int etalon_columns, bool visualize)
{
  if (image->width == 0)
    return false;
  ...
  if (visualize)
  {
    cvDrawChessboardCorners(image, cvSize(etalon_rows, etalon_columns), corners, point_count,
                            false /*pattern_was_found*/);
  }
  delete[] corners;
  cvReleaseImage(&gray);
  if (point_count > 0)
...
bool ProjPoints::AddPointsUsingMarkers(vector<PointDouble>& marker_corners, vector<PointDouble>& marker_corners_img,
                                       IplImage* image)
{
  width = image->width;
  height = image->height;
...
void Camera::Calibrate(ProjPoints& pp)
{
  ...
  cvCalibrateCamera2(object_points, image_points, &point_counts, cvSize(pp.width, pp.height), &calib_K, &calib_D, 0, 0,
...
void Camera::GetOpenglProjectionMatrix(double proj_matrix[16], const int width, const int height,
                                       const float far_clip /*= 1000.0f*/, const float near_clip /*= 0.1f*/)
{
...
void Camera::ProjectPoints(const CvMat* object_points, const CvMat* rotation_vector, const CvMat* translation_vector,
