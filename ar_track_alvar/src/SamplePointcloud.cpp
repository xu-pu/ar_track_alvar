...
void videocallback(IplImage* image)
{
  ...
  if (init)
  {
    ...
    {
      std::cout << "Couldn't load mmarker.xml. Using default "
                   "'SampleMultiMarker' setup."
                << std::endl;
      Pose pose;
      pose.Reset();
    ...
    for (; iter != sfm->container_triangulated.end(); iter++)
    {
      if (iter->second.has_p3d)
      {
        if (own_drawable_count < 1000)
        {
          ...
          d_points[own_drawable_count].gl_mat[14] = iter->second.p3d.z;
          /*if (iter->second.type_id == 0)
          d_points[own_drawable_count].SetColor(1,0,1); else*/
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  This example shows simple structure from motion approach "
                 "that can be "
              << std::endl;
    std::cout << "  used to track environment beyond an multimarker setup. To "
                 "get this   "
              << std::endl;
    std::cout << "  example work properly be sure to calibrate your camera and "
                 "tune it   "
              << std::endl;
    std::cout << "  to have fast framerate without motion blur.                "
              ...
              << std::endl;
    std::cout << std::endl;
    std::cout << "  There are two possible approaches Update() and "
                 "UpdateRotationsOnly()."
              << std::endl;
    std::cout << "  By default the Update() is used but you can easily "
                 "uncomment the     "
              << std::endl;
