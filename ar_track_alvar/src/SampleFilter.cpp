...
void (*(filters[nof_filters]))(double x, double y, double* fx, double* fy) = {
  filter_none, filter_average, filter_median, filter_running_average,
  filter_des,  filter_kalman,  filter_ekf,    filter_array_average,
};
char filter_names[nof_filters][64] = {
  "No filter - Press any key to change", "Average", "Median",          "Running Average",
  "Double Exponential Smoothing",        "Kalman",  "Extended Kalman", "Array (average)"
};
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  This is an example of how to use the 'FilterAverage', "
                 "'FilterMedian',"
              << std::endl;
    std::cout << "  'FilterRunningAverage', "
                 "'FilterDoubleExponentialSmoothing', 'Kalman'"
              << std::endl;
    std::cout << "  'KalmanEkf' and 'FilterArray' filtering classes. First the "
                 "example"
              << std::endl;
    std::cout << "  shows unfiltered test data with outliers. The data is then "
                 "filtered"
              << std::endl;
    std::cout << "  using the various filters. Press any key to cycle through "
                 "the filters."
              << std::endl;
    std::cout << std::endl;
