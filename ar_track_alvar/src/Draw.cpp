...
void DrawCVEllipse(IplImage* image, CvBox2D& ellipse, CvScalar color, bool fill /*=false*/, double par)
{
  ...
  cvEllipse(image, center,
            cvSize(static_cast<int>(par + ellipse.size.width / 2), static_cast<int>(par + ellipse.size.height / 2)),
            -ellipse.angle, 0, 360, color, type);
}

void BuildHideTexture(IplImage* image, IplImage* hide_texture, Camera* cam, double gl_modelview[16],
...
void DrawTexture(IplImage* image, IplImage* texture, Camera* cam, double gl_modelview[16], PointDouble topleft,
