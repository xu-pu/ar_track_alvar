...
void DrawPoints(IplImage* image, const vector<CvPoint>& points, CvScalar color)
{
  for (unsigned i = 0; i < points.size(); ++i)
    cvLine(image, cvPoint(points[i].x, points[i].y),
           cvPoint(points[i].x, points[i].y), color);
}
...
void DrawCircles(IplImage* image, const CvSeq* contour, int radius,
                 CvScalar color)
{
  for (int i = 0; i < contour->total; ++i)
  {
...
void DrawLines(IplImage* image, const CvSeq* contour, CvScalar color)
{
  if (contour->total >= 2)
  {
    for (int i = 0; i < contour->total; ++i)
    {
      ...
      CvPoint* pt2 =
          (CvPoint*)cvGetSeqElem(contour, (i + 1) % (contour->total));
      cvLine(image, cvPoint(pt1->x, pt1->y), cvPoint(pt2->x, pt2->y), color);
    }
  }
}

void DrawCVEllipse(IplImage* image, CvBox2D& ellipse, CvScalar color,
                   bool fill /*=false*/, double par)
{
  CvPoint center;
  ...
  cvEllipse(image, center,
            cvSize(static_cast<int>(par + ellipse.size.width / 2),
                   static_cast<int>(par + ellipse.size.height / 2)),
            -ellipse.angle, 0, 360, color, type);
}

void BuildHideTexture(IplImage* image, IplImage* hide_texture, Camera* cam,
                      ...
                      PointDouble botright)
{
  ...
  double ls = height * ky;
  double r, s;
  double xstep = 2 * objx / hide_texture->width,
         ystep = 2 * objy / hide_texture->height;
  for (i = 0; i < hide_texture->width; i++)
  {
    ...
    for (j = 0; j < hide_texture->height; j++)
    {
      ...
      int opaque;
      const int w = 1;
      if ((i < w) | (j < w) | (i > hide_texture->width - w) |
          (j > hide_texture->width - w))
        opaque = 60;
      else if ((i < 2 * w) | (j < 2 * w) | (i > hide_texture->width - 2 * w) |
               (j > hide_texture->width - 2 * w))
        opaque = 100;
      else if ((i < 3 * w) | (j < 3 * w) | (i > hide_texture->width - 3 * w) |
               (j > hide_texture->width - 3 * w))
        opaque = 140;
      else if ((i < 4 * w) | (j < 4 * w) | (i > hide_texture->width - 4 * w) |
               (j > hide_texture->width - 4 * w))
        opaque = 200;
      else
        opaque = 255;

      cvSet2D(hide_texture, j, i,
              cvScalar((((lr - r) * pixels[7][0] + r * pixels[6][0] +
...
void DrawTexture(IplImage* image, IplImage* texture, Camera* cam,
                 double gl_modelview[16], PointDouble topleft,
                 PointDouble botright)
{
  assert(image->origin == 0);  // Currently only top-left origin supported
  ...
  {  // ttesis: why must we copy the texture first?
    for (int i = 0; i < texture->width; i++)
    {
      ...
      cvSet2D(img, j, i, s);
      if ((i > 0) && (j > 0) && (i < (texture->width - 1)) &&
          (j < (texture->height - 1)))
        cvSet2D(mask, j, i, cvScalar(1));  // ttesis: why are edges not
                                           // included?
    }
  }
