...
bool CaptureDSCapture::start()
{
  if (m_pDSCapture)
  {
    ...
    // We must connect filters before we can get connected media type from
    // grabber(s)
    if (SUCCEEDED(hr))
    ...
    if (SUCCEEDED(hr))
    {
      ...
      if (SUCCEEDED(hr))
      {
        ...
        if ((mt.formattype == FORMAT_VideoInfo) && (mt.cbFormat >= sizeof(VIDEOINFOHEADER)) && (mt.pbFormat != NULL))
        {
          ...
          // "<<pVih->bmiHeader.biBitCount<<" bpp "<<fps<<" fps";
...
CapturePluginDSCapture::CapturePluginDSCapture(const string& captureType) : CapturePlugin(captureType)
{
}
