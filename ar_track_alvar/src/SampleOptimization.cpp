...
{
  double xx = random(CV_RAND_UNI, -(poly_res / 2),
                     +(poly_res / 2));  //(rand()*poly_res/RAND_MAX)-(poly_res/2);
  double yy = get_y(xx, a, b, c, d, e);
  double ry = random(CV_RAND_NORMAL, 0,
                     poly_res / 8);  //(rand()*(poly_res/4)/RAND_MAX)-(poly_res/8);
  yy += ry;
  *x = xx;
  *y = yy;
  if (*y < -(poly_res / 2))
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  This is an example of how to use the 'Optimization' class. "
                 "Random data"
              << std::endl;
    std::cout << "  is generated and approximated using curves of increasing "
                 "degrees."
              << std::endl;
    std::cout << std::endl;
