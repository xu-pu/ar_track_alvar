...
void videocallback(IplImage* image)
{
  ...
  for (size_t i = 0; i < marker_detector.markers->size(); i++)
  {
    ...
    // TODO: Better  option might be to use OpenGL projection matrix that
    // matches our OpenCV-approach
    Pose p = (*(marker_detector.markers))[i].pose;
    BuildHideTexture(image, hide_texture, &cam, d[i].gl_mat, PointDouble(-margin, -margin),
                     PointDouble(margin, margin));
    // DrawTexture(image, hide_texture, &cam, d[i].gl_mat, PointDouble(-0.7,
    // -0.7), PointDouble(0.7, 0.7));
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  This is an example of how to detect 'MarkerData' markers, "
                 "similarly"
              << std::endl;
    std::cout << "  to 'SampleMarkerDetector', and hide them using the "
                 "'BuildHideTexture'"
              << std::endl;
