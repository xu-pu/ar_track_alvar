...
void videocallback(IplImage* image)
{
  ...
  if (!calibrated)
  {
    ...
    // - For every 1.5s add calibration data from detected 7*9 chessboard (and
    // visualize it if true)
    else
    {
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  This is an example of how to use the 'Camera' and "
                 "'ProjPoints' classes"
              << std::endl;
    std::cout << "  to perform camera calibration. Point the camera to the "
                 "chessboard"
              << std::endl;
    std::cout << "  calibration pattern (see ALVAR.pdf) from several "
                 "directions until 50"
              << std::endl;
    std::cout << "  calibration images are collected. A 'calib.xml' file that "
                 "contains the"
              << std::endl;
    std::cout << "  internal parameters of the camera is generated and can be "
                 "used by other"
              << std::endl;
