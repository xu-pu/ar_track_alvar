...
void videocallback(IplImage* image)
{
  ...
  {
    cvLine(image, cvPoint(int(cvGet2D(img_ver, y - 1, 0).val[0]), y - 1),
           cvPoint(int(cvGet2D(img_ver, y, 0).val[0]), y), CV_RGB(255, 0, 0));
  }
  for (int x = 1; x < image->width; x++)
  {
    cvLine(image, cvPoint(x - 1, int(cvGet2D(img_hor, 0, x - 1).val[0])),
           cvPoint(x, int(cvGet2D(img_hor, 0, x).val[0])), CV_RGB(0, 255, 0));
  }
  ...
  for (int y=0; y<image->height/4; y++) {
      ...
      for (int x=0; x<image->width/4; x++) {
          ...
          cvLine(img_grad, cvPoint(r.x+2,r.y+2),
  cvPoint(r.x+2+int(dirx),r.y+2+int(diry)), CV_RGB(255,0,0));
      }
  }
  */
  ...
  for (r.y = 0; r.y < img_canny->height - 4; r.y++)
  {
    for (r.x = 0; r.x < img_canny->width - 4; r.x++)
    {
      if (img_canny->imageData[r.y * img_canny->widthStep + r.x])
      {
        double dirx, diry;
        grad.GetAveGradient(r, &dirx, &diry);
        cvLine(img_grad, cvPoint(r.x + 2, r.y + 2), cvPoint(r.x + 2 + int(dirx), r.y + 2 + int(diry)),
               CV_RGB(0, 0, 255));
        cvLine(img_grad, cvPoint(r.x + 2, r.y + 2), cvPoint(r.x + 2 + int(-diry), r.y + 2 + int(+dirx)),
               CV_RGB(255, 0, 0));
        cvLine(img_grad, cvPoint(r.x + 2, r.y + 2), cvPoint(r.x + 2 + int(+diry), r.y + 2 + int(-dirx)),
               CV_RGB(255, 0, 0));
      }
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  'IntegralGradient' classes. The vertical (green) and "
                 "horizontal (red)"
              << std::endl;
    std::cout << "  whole image projections are computed using "
                 "'IntegralImage::GetSubimage'"
              << std::endl;
    std::cout << "  and shown in the SampleIntegralImage window. The gradients "
                 "of the"
              << std::endl;
    std::cout << "  image edges are shown in the Gradient window. The edges "
                 "are detected"
              << std::endl;
    std::cout << "  using the Canny edge detector where t1 and t2 are "
                 "parameters for the"
              << std::endl;
    std::cout << "  Canny algorithm. The gradients are drawn in red and their "
                 "local normals"
              << std::endl;
