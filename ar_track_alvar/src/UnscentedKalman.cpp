...
void UnscentedKalman::initialize()
{
  ...
  for (int i = 0; i < 5; i++)
    printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(stateCovariance, 0, i),
           cvGetReal2D(stateCovariance, 1, i), cvGetReal2D(stateCovariance, 2, i), cvGetReal2D(stateCovariance, 3, i),
  ...
  for (int i = 0; i < 5; i++)
    printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(sqrtStateCovariance, 0, i),
           cvGetReal2D(sqrtStateCovariance, 1, i), cvGetReal2D(sqrtStateCovariance, 2, i),
           cvGetReal2D(sqrtStateCovariance, 3, i), cvGetReal2D(sqrtStateCovariance, 4, i));
  cvGEMM(sqrtStateCovariance, sqrtStateCovariance, 1., NULL, 0, stateTmp);
  printf("sqrtStateCovariance^2:\n");
  for (int i = 0; i < 5; i++)
    printf("%+.10f %+.10f %+.10f %+.10f %+.10f\n", cvGetReal2D(stateTmp, 0, i), cvGetReal2D(stateTmp, 1, i),
           cvGetReal2D(stateTmp, 2, i), cvGetReal2D(stateTmp, 3, i), cvGetReal2D(stateTmp, 4, i));
#endif
...
void UnscentedKalman::update(UnscentedObservation* obs)
{
  ...
  for (int i = 0; i < sigma_n; i++)
  {
    ...
    cvGEMM(stateDiff, &predObsDiff, scale, &statePredObsCrossCorrelation, 1., &statePredObsCrossCorrelation,
           CV_GEMM_B_T);
  }
