...
void Marker::VisualizeMarkerPose(IplImage* image, Camera* cam, double visualize2d_points[12][2], CvScalar color) const
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    cvLine(image, cvPoint((int)visualize2d_points[4 + i][0], (int)visualize2d_points[4 + i][1]),
           cvPoint((int)visualize2d_points[4 + ((i + 1) % 4)][0], (int)visualize2d_points[4 + ((i + 1) % 4)][1]),
           color);
  }
  // Coordinates
...
void Marker::VisualizeMarkerContent(IplImage* image, Camera* cam, double datatext_point[2],
                                    double content_point[2]) const
{
  ...
  val << int(GetId());
  cvPutText(image, val.str().c_str(), cvPoint((int)datatext_point[0], (int)datatext_point[1]), &font,
            CV_RGB(255, 255, 0));
...
void Marker::VisualizeMarkerError(IplImage* image, Camera* cam, double errortext_point[2]) const
{
  ...
  if (GetError(MARGIN_ERROR | DECODE_ERROR) > 0)
  {
    ...
    cvPutText(image, val.str().c_str(), cvPoint((int)errortext_point[0], (int)errortext_point[1]), &font,
              CV_RGB(255, 0, 0));
  }
...
void MarkerData::VisualizeMarkerContent(IplImage* image, Camera* cam, double datatext_point[2],
                                        double content_point[2]) const
{
#ifdef VISUALIZE_MARKER_POINTS
  for (size_t i = 0; i < marker_allpoints_img.size(); i++)
...
void Marker::CompareContent(vector<PointDouble>& _marker_corners_img, IplImage* gray, Camera* cam,
...
bool Marker::UpdateContentBasic(vector<PointDouble>& _marker_corners_img, IplImage* gray, Camera* cam,
                                int frame_no /*= 0*/)
{
...
void Marker::UpdatePose(vector<PointDouble>& _marker_corners_img, Camera* cam, int orientation, int frame_no /* =0 */,
...
void Marker::ScaleMarkerToImage(IplImage* image) const
{
  ...
  IplImage* img = cvCreateImage(
      cvSize(int(multiplier * (res + margin + margin) + 0.5), int(multiplier * (res + margin + margin) + 0.5)),
      IPL_DEPTH_8U, 1);
  IplImage* img_content =
      cvCreateImage(cvSize(int(multiplier * res + 0.5), int(multiplier * res + 0.5)), IPL_DEPTH_8U, 1);
  cvZero(img);
  CvMat submat;
  cvGetSubRect(img, &submat,
               cvRect(int(multiplier * margin + 0.5), int(multiplier * margin + 0.5), int(multiplier * res + 0.5),
...
bool MarkerData::UpdateContent(vector<PointDouble>& _marker_corners_img, IplImage* gray, Camera* cam,
                               int frame_no /*= 0*/)
{
  if (res == 0)
  {
