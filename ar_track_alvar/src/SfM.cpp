...
public:
  ...
  bool UpdateDistance(Pose* pose, double limit = 10)
  {
    ...
    double dist = (prev[0] - trad[0]) * (prev[0] - trad[0]) + (prev[1] - trad[1]) * (prev[1] - trad[1]) +
...
void CreateShadowPoint(CvPoint3D32f& p3d_sh, CvPoint3D32f p3d, CameraEC* cam, Pose* camera_pose, float parallax_length,
                       float triangulate_angle)
{
  ...
  float shadow_point_dist =
      float(pl / std::abs(tan(triangulate_angle * 3.1415926535 / 180.)));  // How far we need the shadow point to
...
bool SimpleSfM::Update(IplImage* image, bool assume_plane, bool triangulate, float reproj_err_limit,
                       float triangulate_angle)
{
    ...
    while (iter != container_triangulated.end())
    {
      if (container.find(iter->first) == container.end())
      {
        ...
        if ((iter->second.projected_p2d.x >= 0) && (iter->second.projected_p2d.y >= 0) &&
            (iter->second.projected_p2d.x < image->width) && (iter->second.projected_p2d.y < image->height))
        {
