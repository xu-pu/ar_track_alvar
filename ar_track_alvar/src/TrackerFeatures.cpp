...
TrackerFeatures::TrackerFeatures(int _max_features, int _min_features, double _quality_level, double _min_distance,
...
{
  if (_max_features == max_features && _min_features == min_features && _quality_level == quality_level &&
...
double TrackerFeatures::TrackHid(IplImage* img, IplImage* new_features_mask, bool add_features)
{
  ...
  {
    // Track
    cvCalcOpticalFlowPyrLK(prev_gray, gray, prev_pyramid, pyramid, prev_features, features, prev_feature_count,
                           cvSize(win_size, win_size), pyr_levels, status, 0,
...
int TrackerFeatures::AddFeatures(IplImage* new_features_mask)
{
  ...
  if (feature_count < min_features)
  {
    ...
    if (new_features_mask == NULL)
    {
      ...
      {
        cvRectangle(mask, cvPoint(int(features[i].x - min_distance), int(features[i].y - min_distance)),
                    cvPoint(int(features[i].x + min_distance), int(features[i].y + min_distance)), cvScalar(0),
                    CV_FILLED);
      }
      // Find new features
      cvGoodFeaturesToTrack(gray, img_eig, img_tmp, &(features[feature_count]), &new_feature_count, quality_level,
                            min_distance, mask, 3, 1, 0.04);
    }
    else
    {
      cvGoodFeaturesToTrack(gray, img_eig, img_tmp, &(features[feature_count]), &new_feature_count, quality_level,
                            min_distance, new_features_mask, 3, 1, 0.04);
    }
