...
void track_psa_rot(IplImage* image, IplImage* img_gray)
{
  ...
  cvLine(image, cvPoint(int(x), int(y)), cvPoint(int(x - sin(r_rad) * 15), int(y + cos(r_rad) * 15)),
         CV_RGB(255, 0, 0));
}
...
void track_features(IplImage* image, IplImage* img_gray)
{
  ...
  for (int i = 0; i < tracker_features.feature_count; i++)
  {
    cvCircle(image, cvPoint(int(tracker_features.features[i].x), int(tracker_features.features[i].y)), 2,
             CV_RGB(tracker_features.ids[i] % 255, (tracker_features.ids[i] * 7) % 255,
...
void (*(trackers[nof_trackers]))(IplImage* image, IplImage* img_gray) = {
  track_none, track_psa, track_psa_rot, track_stat, track_stat_rot, track_features,
};
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    std::cout << "  This is an example of how to use the 'TrackerPsa', "
                 "'TrackerPsaRot',"
              << std::endl;
    std::cout << "  'TrackerFeatures', 'TrackerStat' and 'TrackerStatRot' "
                 "classes to"
              << std::endl;
