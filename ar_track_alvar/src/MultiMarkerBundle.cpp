...
void Est(CvMat* state, CvMat* estimation, void* param)
{
  ...
  for (int i = 0; i < n_images; ++i)
  {
    ...
    for (int j = 0; j < n_points; ++j)
    {
      ...
      cvProjectPoints2(&mat_object_points, &mat_rotation_vector, &mat_translation_vector, &(camera->calib_K),
...
bool MultiMarkerBundle::Optimize(Camera* _cam, double stop, int max_iter, Optimization::OptimizeMethod method)
{
  ...
  cout << "Optimizing with " << optimization_keyframes << " keyframes and " << optimization_markers << " markers"
       << endl;
